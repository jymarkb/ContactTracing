...
$(document).ready(function(){
    ...
    $("#eluaText").on('scroll', function() {
        var txtScrollTop = this.scrollTop;
        var txtOffsetH = this.offsetHeight;
        var txtScrollH = this.scrollHeight;
        if ((txtScrollTop + txtOffsetH +1 ) > txtScrollH){
...
async function sendRequest(){
    ...
    $.ajax({
        ...
        success: function(data){
            console.log(data);
            if(data == "true"){
                window.location = "/individual/verification";
            }
        }
   });
